...
function! dbext#DB_selectFromTableWithWhere(...)
    let object = s:DB_retrieveObjectName(a:000)
    return dbext#DB_execSql("select * from " .  s:DB_applyTableNameConvention(object) . " where " .
...
function! s:DB_retrieveObjectName(args)
    if(!empty(a:args))
        let result = a:args[0]
    else
        let result = expand("<cword>")
    endif
    return s:DB_getObjectAndQuote(result)
endfunction
